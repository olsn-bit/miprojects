...
public class linux {
    public static void main(String[] args) throws AWTException, IOException, UnsupportedFlavorException {
        int x = 0;
        File js = new File("/home/alumne/Dropbox/Programming/JavaScript/justvibing/whatsappTranslatorBot.js");
        Scanner sc = new Scanner(js);
        ...
        while(x < 1) {
            ...
            robot.mouseMove(1038,588);//f12 last
            doubleclick(robot);
//            robot.mousePress(InputEvent.BUTTON1_DOWN_MASK);
//            robot.mouseRelease(InputEvent.BUTTON1_DOWN_MASK);
            robot.delay(2000); //delay 2 second
            String command = robotcopy(robot);
            robot.mouseMove(450,684);//input bar
            robot.mousePress(InputEvent.BUTTON1_DOWN_MASK);
            robot.mouseRelease(InputEvent.BUTTON1_DOWN_MASK);
            robot.delay(2000); //delay 2 second
