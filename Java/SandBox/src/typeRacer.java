...
public class typeRacer {
    ...
    private Robot robot;
    public static final int delayBetweenTypes = 0;
	public static String[] loadFromFile() throws IOException {
	    String imported = new String(Files.readAllBytes(Paths.get("src/data.txt")));
	    String[] toReturn = new String[10];
        int start = 0;
        int end;
        for (int i = 0; i < toReturn.length; i++) {
            end = start+5;
            toReturn[i] = imported.substring(start,end);
    ...
    public static void main(String... args) throws Exception {
        Robot r = new Robot();
        String[] words = loadFromFile();
        int counter = 0;
        while (counter < words.length){
            StringSelection stringSelection = new StringSelection(words[counter]);
    ...
    public typeRacer() throws AWTException {
        this.robot = new Robot();
    }

    public typeRacer(Robot robot) {
        this.robot = robot;
    }
